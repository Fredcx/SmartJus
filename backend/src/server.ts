...
// Middleware para logar tamanho da requisição
app.use((req, res, next) => {
  if (req.method === 'POST') {
    const size = parseInt(req.headers['content-length'] || '0');
    if (size > 0) {
      console.log(`📥 REQUISIÇÃO POST: ${req.path} | Tamanho: ${(size / 1024).toFixed(2)} KB`);
    }
  }
  next();
});
...
app.use('/api/v1/gateway-record', uploadRoutes);
